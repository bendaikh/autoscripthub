...
<body>
    ...
    <div id="right-panel" class="right-panel">
                     ...
                     @else
                     <form action="{{ route('admin.post') }}" method="post" id="setting_form" enctype="multipart/form-data">
                     {{ csrf_field() }}
                     @endif
                                ...
                                <table id="example" class="table table-striped table-bordered">
                                    ...
                                    @foreach($postData['post'] as $post)
                                        <tr class="allChecked">
                                            ...
                                            <td><a href="{{ url('/admin') }}/comment/{{ $post->post_id }}" class="blue-color">{{ __('Comments') }} [{{ $comments->has($post->post_id) ? count($comments[$post->post_id]) : 0 }}]</a></td>
                                            ...
                                            <td><a href="{{ url('/admin') }}/edit-post/{{ $post->post_id }}" class="btn btn-success btn-sm"><i class="fa fa-edit"></i>&nbsp; {{ __('Edit') }}</a> 
                                            @if($demo_mode == 'on') 
                                            <a href="{{ url('/admin') }}/demo-mode" class="btn btn-danger btn-sm"><i class="fa fa-trash"></i>&nbsp;{{ __('Delete') }}</a>
                                            @else
                                            <a href="{{ url('/admin') }}/post/{{ $post->post_id }}" class="btn btn-danger btn-sm" onClick="return confirm('{{ __('Are you sure you want to delete') }}?');"><i class="fa fa-trash"></i>&nbsp;{{ __('Delete') }}</a>@endif</td>
                                        </tr>
